...
import (
	...
	trxkit "github.com/likearthian/apikit/v2/transport"
)

var (
	// ErrNilEndpoint is returned when an HTTP server is constructed without an endpoint.
	ErrNilEndpoint = errors.New("http server endpoint is required")
	// ErrNilRequestDecoder is returned when an HTTP server is constructed without a request decoder.
	ErrNilRequestDecoder = errors.New("http server request decoder is required")
	// ErrNilResponseEncoder is returned when an HTTP server is constructed without a response encoder.
	ErrNilResponseEncoder = errors.New("http server response encoder is required")
)
...
func NewServer[I, O any](
	...
	if dec == nil {
		return nil, ErrNilRequestDecoder
	}
	if enc == nil {
		return nil, ErrNilResponseEncoder
	}
	...
	s := &Server[I, O]{
		...
		errorHandler: trxkit.NewNopErrorHandler(),
		before:       opts.before,
		after:        opts.after,
